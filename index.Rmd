...
norm.mutiple = function(data, group){
  ...
  T = (sum(n.vec)-1) * sum(tapply(norm.score, w[, 2], 
                                  function(x) sum(x)^2/length(x))) / sum(norm.score^2)
  p = pchisq(T, length(unique(group))-1)
  list(H1='H1: Not all the accumulate functions are identical', 
       p.value = min(p, 1-p))
}
```
---
...
```{r}
Friedman = function(data){ # 行表示处理,列代表区组
  if ((is.matrix(data) | is.data.frame(data)) == FALSE) stop("input should be a matrix or a dataframe")
  k = nrow(data); b = ncol(data)
  r = apply(m, 2, rank)
...
cochran.test = function(data){
  ...
  n = colSums(data); l = rowSums(data)
  N = sum(n); k = ncol(data)
  Q = k*(k-1)*sum((n-mean(n))^2) / (k*N - sum(l^2))
  list(H1='Not all the individuals are regarded equally', p.value=pchisq(Q, k-1, low=F))
}
```
...
page.test = function(data){
  ...
  R = rowSums(r)
  L = sum(R*1:length(R))
  k = ncol(data); b = nrow(data)
  u = b*k*(k+1)^2/4
  tie = as.vector(r) %% 1 == 0
  s = ifelse(all(tie), b*(k^3-k)^2,
             k*(k^2-1)*(b*k*(k^2-1)-sum(as.vector(r)[!tie]^3 - as.vector(r)[!tie])))
  Z = (L - u)/sqrt(s/144/(k-1))
  list(H1='Not all the infulences of diffierent processings are the same' , p.value=pnorm(Z, low=F))
}
```
